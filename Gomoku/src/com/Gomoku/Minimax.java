...
public class Minimax {
    ...
    // Constructor
    public Minimax(OfflineBoard board) {
        this.board = board;
    }
    ...
    public static double evaluateBoardForWhite(OfflineBoard board, boolean blacksTurn) {
        evaluationCount++;
    ...
    public static int getScore(OfflineBoard board, boolean forBlack, boolean blacksTurn) {
    ...
    public int[] calculateNextMove(int depth) {
        ...
        if(bestMove != null ) {
            ...
            System.out.println("Best Move Found!" + bestMove[1] + " " + bestMove[2]);
            move[0] = (Integer)(bestMove[1]);
            move[1] = (Integer)(bestMove[2]);

        } else {
            // If there is no such move, search the minimax tree with specified depth.
            bestMove = minimaxSearchAB(depth, new OfflineBoard(board), true, -1.0, getWinScore());
            if(bestMove[1] == null) {
                move = null;
            } else {
                move[0] = (Integer)(bestMove[1]);
     ...
     * */
    private static Object[] minimaxSearchAB(int depth, OfflineBoard dummyBoard, boolean max, double alpha, double beta) {
        ...
        if(max) {
            ...
            for(int[] move : allPossibleMoves) {
                ...
                // Print the board
//				int[][] tem_boardMatrix = dummyBoard.getBoardMatrix();
//				System.out.println("Dummy Board:");
//				for(int i=0; i<tem_boardMatrix.length; i++) {
//					for(int j=0; j<tem_boardMatrix[0].length; j++) {
//						System.out.print(tem_boardMatrix[i][j] + " ");
//					}
//					System.out.println();
//				}
    ...
    private static Object[] searchWinningMove(OfflineBoard board) {
        ...
        // In ra các Possible Move
        for(int[] move1 : allPossibleMoves) {
            System.out.println("Move: " + move1[0] + " " + move1[1]);
        }
        ...
        for(int[] move : allPossibleMoves) {
            evaluationCount++;
            // Create a temporary board that is equivalent to the current board
            OfflineBoard dummyBoard = new OfflineBoard(board);
            // Play the move on that temporary board without drawing anything
            dummyBoard.addStoneNoGUI(move[1], move[0], false);
